...
class Trainer(object):
    def __init__(self, configer):
        configer.update(("solver", "max_iters"), configer.get("solver", "max_iters_pre"))
    ...
    def _init_classifier_training(self):
        self.configer.update(("solver", "pretraining"), False)
        self.module_runner.save_net(self.seg_net, save_mode='iters', experiment=None)
        self.seg_net.module.init_seg_training()
        self.configer.update(("solver", "max_iters"), self.configer.get("solver", "max_iters_seg"))
        self.configer.update(("iters",), 0)
        if self.configer.exists("lr", "seg_lr"):
            self.configer.update(("lr", "base_lr"), self.configer.get("lr", "seg_lr"))
    ...
    def __train(self, pretraining=True):
        """
        ...
        for i, data_dict in enumerate(self.train_loader):
            ...
            if self.seg_net.module.pretraining:
                self.summary_writer.add_scalar("Loss/Contrast", display_loss, n_iter)
            else:
                self.summary_writer.add_scalar("Loss/CE", display_loss, n_iter)

            try:
                self.summary_writer.add_scalar("Learning Rate", self.module_runner.get_lr(self.optimizer)[0], n_iter)
            except:
                Log.info('Tensorboard: Could not plot learning rate')
            ...
            if self.configer.get('iters') % self.configer.get('solver', 'test_interval') == 0:
                self.module_runner.save_net(self.seg_net, save_mode='iters', experiment=None)
                if not self.seg_net.module.pretraining:
                    self.__val()
    ...
    def train(self):
        ...
        if self.configer.get("solver", "pretraining"):
            while self.configer.get('iters') < self.configer.get('solver', 'max_iters_pre'):
                self.__train()

        Log.info('Pretraining complete. Starting classifier training')
        self._init_classifier_training()

        while self.configer.get('iters') < self.configer.get('solver', 'max_iters_seg'):
            self.__train()
