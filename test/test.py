...
from lib.loss.loss_au import AULoss
...
img_name = 'frankfurt_000000_001236_leftImg8bit'
label_dict = {7:0, 8:1, 11:2, 12:3, 13:4, 17:5, 19:6, 20:7, 21:8, 22:9, 23:10, 24:11, 25:12, 26:13, 27:14, 28:15, 31:16, 32:17, 33:18}

with Image.open(f"./cityscapes_test/val/label/{img_name}.png") as labelmap:
    arr = np.array(labelmap)
    arr_remapped = np.copy(arr)
    unique = np.unique(arr)
    for label in unique:
        if label in label_dict:
            arr_remapped[arr == label] = label_dict[label]
        else:
            arr_remapped[arr == label] = 19
    labelmap = Image.fromarray(arr_remapped)
    labelmap.putpalette(colors)
    labelmap.save(f"./labelmaps/{img_name}.png")