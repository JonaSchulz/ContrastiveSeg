...
# calculate relative occurrences per class
class_rel = {}
size = 1024 * 2048
for i, data in enumerate(val_loader):
    ...
    prob_total = 0
    for cls in classes:
        freq = torch.count_nonzero(label == cls) / size
        prob_total += freq
        cls_int = cls.item()
        if cls_int not in class_rel:
            class_rel[cls_int] = torch.zeros(len(val_loader))
            class_rel[cls_int][i] = freq
        else:
            class_rel[cls_int][i] = freq
    assert prob_total == 1, f"Prob Total: {prob_total}"

for cls, freq in class_rel.items():
    class_rel[cls] = torch.mean(class_rel[cls])
...
for i in class_labels:
    bar_heights.append(class_rel[i])

x_ticks = list(range(len(bar_heights)))
x_tick_labels = class_labels.values()
