from lib.loss.loss_contrast_mem import PixelContrastLoss
...
loss = AULoss(configer)
feat = torch.randn((2, 3, 2)).to("cuda")
label = torch.tensor([[1, 2]]).to("cuda")
predict = torch.randint(1, 4, (1, 20)).to("cuda")

print(loss._contrastive(feat, label))
print(f"Feat:\n{feat}")
print(f"Label:\n{label}")
print(f"Predict:\n{predict}")
