...
def handle_distributed(args, main_file):
    ...
    try:
        main_index = command_args.index(main_file)
    except:
        try:
            main_index = command_args.index("main_contrastive.py")
        except:
            main_index = command_args.index("main_contrastive_seq.py")
        # main_index = command_args.index('main.py')

    ports = [29961, 29962, 29963, 29964, 29965]
    for port in ports:
        command_args_ = command_args[main_index+1:]
        command_args_ = [
            python_exec, '-u',
        ...
        process = subprocess.Popen(command_args_, env=current_env)
        process.wait()
