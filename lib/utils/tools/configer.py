...
class Configer(object):
    """
    attributes:
        params_root: config dictionary (contains hyperparameters)
        args_dict: contains all arguments of args_parser (only if args_parser argument passed to __init__)
        conditions:
    """

    def __init__(self, args_parser=None, configs=None, config_dict=None):
        """
        sets params_root, args_dict (if args_parser passed as argument)
        --> creates one dictionary with all parameters of configs and command line arguments
        Args:
            args_parser: ArgumentParser object
            configs: configs json file
            config_dict: configs dictionary
        """

        if config_dict is not None:
            # setting params_root to be the configs dictionary
            ...
            for key, value in self.args_dict.items():
                # if it does not yet exist, add new key:value pair to params_root
    ...
    def get(self, *key):
        if len(key) == 0:
            ...
            if key[0] in self.params_root and key[1] in self.params_root[key[0]] and key[2] in self.params_root[key[0]][key[1]]:
                return self.params_root[key[0]][key[1]][key[2]]
            else:
                Log.error('{} KeyError: {}.'.format(self._get_caller(), key))
                exit(1)
