...
class HRNet_W48_CONTRAST(nn.Module):
    ...
    def __init__(self, configer):
        ...
        self.proj_head = ProjectionHead(dim_in=in_channels, proj_dim=self.proj_dim, bn_type=self.configer.get("network", "bn_type"))

    def forward(self, x_):
        assert torch.is_tensor(x_), f"Not a tensor: {x_}"
        ...
        if self.configer.exists("embedding_visualizer", "encodings") or self.configer.exists('save_features'):
            return {'seg': out, 'embed': emb, 'encode': feats}
...
class HRNet_W48_MEM(nn.Module):
    def __init__(self, configer, dim=256, m=0.999, with_masked_ppm=False):
        ...
        dim = self.configer.get("contrast", "proj_dim")
    ...
    def forward(self, im_q, lb_q=None):
        is_eval = self.configer.get("is_eval")
...
class HRNet_W48_CONTRAST_SEQ(nn.Module):
    def __init__(self, configer):
        super(HRNet_W48_CONTRAST_SEQ, self).__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        self.backbone = BackboneSelector(configer).get_backbone()
        self.proj_dim = self.configer.get('contrast', 'proj_dim')
        ...
        self.proj_head = ProjectionHead(dim_in=720, proj_dim=self.proj_dim, bn_type=self.configer.get("network", "bn_type"))
        self.relu = ModuleHelper.BNReLU(in_channels, bn_type=self.configer.get('network', 'bn_type'))
        self.pretraining = True

    def init_seg_training(self):
    ...
    def forward(self, x_):
        ...
        feats = torch.cat([feat1, feat2, feat3, feat4], 1)
        out = self.proj_head(feats)
        if not self.pretraining:
            # out = self.relu(out)
            out = self.cls_head(out)

        return {"embed": out} if self.pretraining else {"seg": out}
